...
void hienthiDat(DanhSach L) {
	for (int i = 1; i <= L.n; i++) {
		if (L.A[i-1].DiemLT + L.A[i-1].DiemTH1 + L.A[i-1].DiemTH2 >= 4) {
			float markTotal = L.A[i-1].DiemLT	+ L.A[i-1].DiemTH1 + L.A[i-1].DiemTH2;
     		printf("%s - %s - %.2f - %.2f - %.2f - %.2f\n", L.A[i-1].MSSV, L.A[i-1].HoTen, L.A[i-1].DiemLT, L.A[i-1].DiemTH1, L.A[i-1].DiemTH2, markTotal);
		}
	}
}
...
void hienthi(DanhSach L) {
	for(int i = 0; i < L.n; i++) {
		float markTotal = L.A[i].DiemLT	+ L.A[i].DiemTH1 + L.A[i].DiemTH2;
     	printf("%s - %s - %.2f - %.2f - %.2f - %.2f\n", L.A[i].MSSV, L.A[i].HoTen, L.A[i].DiemLT, L.A[i].DiemTH1, L.A[i].DiemTH2, markTotal);
 	}
}
